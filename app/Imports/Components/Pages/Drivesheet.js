...
import { Animated, View, TouchableOpacity, Image, ScrollView, TextInput, Text, Alert } from "react-native";
...
export default function Drivesheet() {
    ...
    const fetchData = async () => {
        ...
        const response = await axios({
            method: 'GET',
            url: 'https://s2.safari-gps.live/api/fleet/trips.php?cmd=load_trip_datas&token=24b181660429dbc38586cbfb66d01bca&imei=355139089843046&dtf=2023-11-25%2009:00&dtt=2023-11-25%2011:00&min_stop_duration=1'
        });
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <TouchableOpacity onPress={() => {
                            isButtonClicked ? setIsButtonClicked(!isButtonClicked) : router.back();
                        }} >
                            <SlideInView trigger={isButtonClicked}

                            >
                                {isButtonClicked ? <Svg fill="#ffffff" height={30} width="30" viewBox="0 0 460.775 460.775" >
                                    <Path d="M285.08,230.397L456.218,59.27c6.076-6.077,6.076-15.911,0-21.986L423.511,4.565c-2.913-2.911-6.866-4.55-10.992-4.55 c-4.127,0-8.08,1.639-10.993,4.55l-171.138,171.14L59.25,4.565c-2.913-2.911-6.866-4.55-10.993-4.55 c-4.126,0-8.08,1.639-10.992,4.55L4.558,37.284c-6.077,6.075-6.077,15.909,0,21.986l171.138,171.128L4.575,401.505 c-6.074,6.077-6.074,15.911,0,21.986l32.709,32.719c2.911,2.911,6.865,4.55,10.992,4.55c4.127,0,8.08-1.639,10.994-4.55 l171.117-171.12l171.118,171.12c2.913,2.911,6.866,4.55,10.993,4.55c4.128,0,8.081-1.639,10.992-4.55l32.709-32.719 c6.074-6.075,6.074-15.909,0-21.986L285.08,230.397z" />
                                </Svg> : <Icon name='arrow-left' size={30} color='#fff' />
                                }
                            </SlideInView>
                ...
                <ScrollView>
                    ...
                    <View style={{
                        ...
                        <View style={{
                            ...
                            {loading ? <Text style={{ textAlign: "center", fontSize: 16 }}>Fetching Data..</Text> : null}
                            {data && data.map((item, index) => {
                                return (
                                    <Suspense key={index}>
                                        <TouchableOpacity style={{ backgroundColor: '#e8e8e8', borderRadius: 5, padding: 15, flexDirection: 'row', display: 'flex', gap: 15, alignItems: "center", justifyContent: 'space-around' }}>
                                            {isButtonClicked && <BouncyCheckbox
                                                size={25}
                                            ...
                                            <View>
                                                <Text style={style.ItemDate}>{formatTime(item.Date)}</Text>
                                                <Text style={style.ItemDate}>{formatTime(item.Date2)}</Text>
                                            </View>
